...
*/
#define REGISTER_OFFSET 0x00200000 /* A21 = High */
#define CONTROL_OFFSET  0x00400000 /* A22 = High */
#define IDE_SHIFT 16
...
static u8 ide_itomap24xx_inb (unsigned long port)
{
    return (u8) readb(port);
}

static u16 ide_itomap24xx_inw (unsigned long port)
{
    return (u16) readw(port);
}

#if defined(CONFIG_DMA_HDD)
static void ide_itomap24xx_insw (unsigned long port, void *addr, u32 count)
{
    volatile u16 *d_ptr;
...
static void ide_itomap24xx_insw (unsigned long port, void *addr, u32 count)
{
    readsw((const void __iomem*)port, addr, count);
}
#endif

static u32 ide_itomap24xx_inl (unsigned long port)
{
    return (u32) readl(port);
}

static void ide_itomap24xx_insl (unsigned long port, void *addr, u32 count)
{
    readsl((const void __iomem*)port, addr, count);
}

static void ide_itomap24xx_outb (u8 val, unsigned long port)
{
    writeb(val, port);
}

static void ide_itomap24xx_outbsync (ide_drive_t *drive, u8 addr, unsigned long port)
{
    writeb(addr, port);
}


static void ide_itomap24xx_outw (u16 val, unsigned long port)
{
    writew(val, port);
}
...
#if defined(CONFIG_DMA_HDD)
static void ide_itomap24xx_outsw (unsigned long port, void *addr, u32 count)
{
    volatile u16 *d_ptr;
...
static void ide_itomap24xx_outsw (unsigned long port, void *addr, u32 count)
{
    writesw((void __iomem*)port, addr, count);
}
#endif

static void ide_itomap24xx_outl (u32 val, unsigned long port)
{
    writel(val, port);
}

static void ide_itomap24xx_outsl (unsigned long port, void *addr, u32 count)
{
    writesl((void __iomem*)port, addr, count);
}

spinlock_t it_hdd_lock = SPIN_LOCK_UNLOCKED;

#ifdef GIO_HDD_HOTPLUG    
static void ide_itomap24xx_hotplug(void)
{
...
static irqreturn_t ide_itomap24xx_hotplug_handler(int i, void * data,
        struct pt_regs * regs)
{
    ide_itomap24xx_hotplug();
    return IRQ_HANDLED;
}
#endif
...
#if defined(CONFIG_DMA_HDD)
void
itomap24xx_ide_wait_dma_complete( void )
{
    while (inw(IO_EMIF_DMACTL) & 0x0001); 
}
#endif

void __init itomap24xx_ide_hdd_probe(void)
{
    int i;
    ide_hwif_t *hwif;
    ...
    hwif->hw.irq                          = OMAP_GPIO_IRQ(OMAP24XX_HDD_GPIO_IRQ);
    hwif->noprobe                         = 0;
    ...
    */
    hwif->OUTB      = ide_itomap24xx_outb;
    hwif->OUTBSYNC  = ide_itomap24xx_outbsync;
    hwif->OUTW      = ide_itomap24xx_outw;
    hwif->OUTL      = ide_itomap24xx_outl;
    hwif->OUTSW     = ide_itomap24xx_outsw;
    hwif->OUTSL     = ide_itomap24xx_outsl;
    hwif->INB       = ide_itomap24xx_inb;
    hwif->INW       = ide_itomap24xx_inw;
    hwif->INL       = ide_itomap24xx_inl;
    hwif->INSW      = ide_itomap24xx_insw;
    hwif->INSL      = ide_itomap24xx_insl;
    
    memcpy(hwif->io_ports, hwif->hw.io_ports, sizeof(hwif->io_ports));
    hwif->irq                             = hwif->hw.irq;
    printk("Ingenient onboard IDE/HDD configured as device %i\n", i);
    // printk("\tstatus=%02x\n", *((volatile char*)__ide_hdd(0x270000)));
#ifdef GIO_HDD_HOTPLUG       
    request_gio(GIO_HDD_HOTPLUG);
    gio_enable_irq(GIO_HDD_HOTPLUG,GIO_ANY_EDGE);
    request_irq(GIO_HDD_HOTPLUG + 21,ide_itomap24xx_hotplug_handler,0,"ide_usb_hotplug",NULL);
#endif
}

...
static int init_irq (ide_hwif_t *hwif)
{
	...
	if (!match || match->irq != hwif->irq) {
     ...
     set_irq_type(hwif->irq, IRQT_RISING);
#endif
...
// ISOFT_PORT
#ifdef CONFIG_ARCH_ITDM3XX
        /* Enable IRQ if on GIO */
        if (hwif->irq >= IRQ_GIO0 && hwif->irq <= IRQ_GIO15)
        {
           if (request_gio(hwif->irq - IRQ_GIO0)) {
			 printk(KERN_ERR "Gio %d, was previously registered!\n",
                     hwif->irq - IRQ_GIO0);
              return -EBUSY;
           }
           gio_enable_irq(hwif->irq - IRQ_GIO0, GIO_RISING_EDGE);
        }
        else /* Must be CFC Interface,  Enable CFC Busy Interrupt on MTC1 */
		  outw(0x0001, IO_EMIF_BUSINTEN);
#endif

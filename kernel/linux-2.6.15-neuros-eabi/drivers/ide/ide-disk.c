...
static int ide_disk_remove(struct device *dev)
{
	...
	//HACK: do not know why, but if IDE layer does not
	//      NULL this queue, somehow FAT layer will not be
	//      able to flush when hotplug removes the node.
	//      see generic_make_request(...)
	//
	// blk_cleanup_queue(g->queue); //this does NOT work.
	g->queue = 0;
	// endofhack ----------------------- 01-26-2007 tq/mg@neuros

...
#if defined(CONFIG_BLK_DEV_ITDM320_20_IDE_HDD) && defined(CONFIG_DMA_HDD)
        /* Wait for RX DMA to complete. */
	/* Read performance increases 50% if we wait here */
	/* instead of inside hwif->INSW() */
	void itdm320_20_ide_wait_dma_complete(void);

        itdm320_20_ide_wait_dma_complete( );
#endif
	/*

...
#define MOD_DESC "Neuros IR Driver (c) 2006"
...
#if USE_WORKQUEUE
static void irrtc_do_wq(void *data)
{       
	int key;
	int retry = IR_RETRY_COUNT;

	if (inw( IO_GIO_IRQPORT) & GIO3)
	{
...
DECLARE_WORK(irrtc_wq, irrtc_do_wq, NULL); 
#endif

static irqreturn_t handle_irrtc_irqs(int irq, void * dev_id, struct pt_regs * regs)
{
#if USE_WORKQUEUE
	if (!factory_test)
	...
	{
		schedule_delayed_work(&irrtc_wq,FACTORY_TEST_DELAY);
		set_factory_test(0);
	}
#else
	keyin=1;
...
{ 
    ...
    if (roffset < woffset)
        copy_to_user(buff, keybuf + roffset, count);
    else {
        copy_to_user(buff, &keybuf[roffset], (KEYBUF_SIZE - roffset + 1) * sizeof(int));
        copy_to_user(buff + (KEYBUF_SIZE - roffset + 1) * sizeof(int), keybuf, woffset * sizeof(int));
    }
...
//-------------------------------------------------- PROC FS ENTRY --------------------------------------------------------
/*
static int irrtc_proc_read(char * page, char ** start, off_t offset, int count, int * eof, void * data)
{
    int key = 0;
    int length = 0;
...
#endif
    length += sprintf(page + length, "IR Key: %d\n", key);
    *eof = 1;
    return length;
}

static void proc_irrtc_init( void )
{
    create_proc_read_entry( "neuros_ir", 0, 0, irrtc_proc_read, &device );
}

static void proc_irrtc_exit( void )
{
    remove_proc_entry("neuros_ir", 0);
}

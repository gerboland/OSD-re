...
//  cmd.flags = MMC_RSP_NONE;
    ...
    if( (gio_get_bitset(GIO_SD_CARDDETECT) & !(inw(IO_GIO_CARD_ST))) == 0)
      {
	mmc_wait_for_cmd(host, &cmd, 0);
      }
    mmc_delay(1);
 ...
 */
static int mmc_send_if_cond(struct mmc_host *host, u32 ocr, int *status)
{
    struct mmc_command cmd;
...
//      host->ocr = mmc_select_voltage(host, ocr);
    ...
    {
        int sdhc;
        err = mmc_send_if_cond(host, host->ocr, &sdhc);
        if(err == MMC_ERR_NONE)
        {
            if(sdhc)
                mmc_send_app_op_cond(host, host->ocr | (1<<30), NULL);
            else
...
int mmc_add_host(struct mmc_host *host)
{
    ...
    //if (ret == 0) {
    //if (ret == -1) {
    //if((ret==0)&&(!(inw(IO_MMC_STATUS1)&0x10))){
    if( (ret==0)&&((gio_get_bitset(GIO_SD_CARDDETECT)&!(inw(IO_GIO_CARD_ST)))!=0) ){
        mmc_power_off(host);
...
static void proc_mmc_sd_init( void )
{

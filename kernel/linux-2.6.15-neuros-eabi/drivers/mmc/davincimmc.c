...
#include "davincimmc.h"
#include <asm/arch/edma.h>

//#define MMC_DEBUG

#define MMCSD_INIT_CLOCK 		150000 			/*MMCSD Init clock in Hz in opendain mode*/
#define MMC_MODULE_INPUT_FREQ	76500000
#define DRIVER_NAME 			"mmc-davinci"
#define MMCINT_INTERRUPT    	DAVINCI_MMCINT_INTERRUPT
#define MMCSD_REGS_BASE_ADDR  	DAVINCI_MMC_SD_BASE_ADDR
...
static void mmc_davinci_start_command(struct mmc_davinci_host *host, struct mmc_command *cmd)
{
	u32 cmdreg =0;
	u32 resptype =0;
	u32 cmdtype =0;
	...
	if((host->use_dma == 1) && (host->data !=NULL) && ((cmd->opcode == 18 )||(cmd->opcode== 25)|| (cmd->opcode== 24) || (cmd->opcode== 17) ))
	{
		cmdreg = cmdreg | (1<< 16);
	}
...
static void mmc_davinci_dma_cb(int lch, u16 ch_status, void *data)
{
...
#ifdef MMC_DEBUG		
			printk("MMC- interrupt from DMA when no request has been made\n");
#endif		
	...
	else
	{
		...
		printk("MMC : Event miss interrupt has been generated by DMA");
		if(host->datadir == DAVINCI_MMC_DATADIR_READ)
		{
			sync_dev=DAVINCI_DMA_MMCTXEVT;	/*Write*/
		}
...
static int mmc_davinci_start_dma_transfer(struct mmc_davinci_host *host, struct mmc_request *req)
{
	const char *dev_name;
...
#define MAX_C_CNT		64000	
	...
	if ((req->data->flags & MMC_DATA_WRITE)) 
	{
		...
		if(dma_write_req)
		{
			r = davinci_request_dma(sync_dev, dev_name, mmc_davinci_dma_cb, host, &edmach,&tcc,queueno);
			if (r != 0) 
			{
				printk("MMC: davinci_request_dma() failed with %d\n",r);
				return r;
			}
			dma_write_req=0;
		}
	...
	davinci_set_dma_transfer_params(sync_dev,acnt,bcnt,ccnt,bcntrld,sync_mode);
	...
	if(num_eight_words > MAX_C_CNT) /* Linking will be performed*/
	{
		...
		for(i =0; i< EDMA_MAX_LOGICAL_CHA_ALLOWED; i++)
		{
			...
			{
				j= i-1;
				davinci_get_dma_params(host->edmaChDetails.chanelNum[j], &temp);
				temp.opt &= ~TCINTEN;
				davinci_set_dma_params(host->edmaChDetails.chanelNum[j],&temp);
			}

			host->edmaChDetails.cntChanel++;
			davinci_request_dma(DAVINCI_EDMA_PARAM_ANY,"LINK",NULL,NULL,&edmachan_num,&sync_dev,queueno);
			...
			{
				temp.ccnt = (temp.ccnt & 0xFFFF0000) | MAX_C_CNT;
				ccnt = temp.ccnt & 0x0000FFFF;
				tempcCnt = tempcCnt + ccnt;
			}
			else
			{
				temp.ccnt = (temp.ccnt & 0xFFFF0000) | (num_eight_words - tempcCnt);
				ccnt = temp.ccnt & 0x0000FFFF;
				tempcCnt = tempcCnt + ccnt;
			}
			...
			{
				j= i-1;
				davinci_dma_link_lch(host->edmaChDetails.chanelNum[j],edmachan_num);
			}
		...
		davinci_dma_link_lch(sync_dev,host->edmaChDetails.chanelNum[0]);
	}
...
static void mmc_davinci_prepare_data(struct mmc_davinci_host *host, struct mmc_request *req)
{
	int timeout;
	host->data = req->data;
	if (req->data == NULL) 
...
#ifdef MMC_DEBUG
	printk( "MMCSD : Data xfer (%s %s), DTO %d cycles + %d ns, %d blocks of %d bytes\r\n",
    ...
    if ((host->use_dma == 1) && (mmc_davinci_start_dma_transfer(host, req) == 0))
    {
		host->buffer = NULL;
		host->bytesleft = 0;
	} 
...
static unsigned int calculate_freq_for_card(unsigned int mmc_req_freq)
{
	...
	cpu_arm_clk = MMC_MODULE_INPUT_FREQ;

	if(cpu_arm_clk > (2*mmc_req_freq))
	{
		mmc_pushpull = ((unsigned int)cpu_arm_clk/(2*mmc_req_freq))-1;
	}
	else
	{
		mmc_pushpull = 0;
...
static void mmc_davinci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    if(ios->bus_mode == MMC_BUSMODE_OPENDRAIN)
    {
	    ...
	    opendrain_freq = ((unsigned int)cpu_arm_clk/(2*MMCSD_INIT_CLOCK))-1;
...
static void mmc_davinci_cmd_done(struct mmc_davinci_host *host, struct mmc_command *cmd)
{
	host->cmd = NULL;
	switch (cmd->flags & MMC_RSP_MASK) 
	{
...
#ifdef MMC_DEBUG			
			printk("MMC :print from code segment excuted when card removed\n");
#endif			
			return IRQ_HANDLED;
		}
	}
	...
	while(status !=0)
	{
		...
		if (status & MMCSD_EVENT_ERROR_DATACRC) 
		{
			...
			if (host->data) 
			{
				...
				printk(KERN_DEBUG "MMCSD: Data CRC error, bytes left %d\r\n", host->bytesleft);
				end_transfer = 1;
			}
			else 
			{
		...
		if (status & MMCSD_EVENT_ERROR_CMDTIMEOUT) 
		{
			...
			if (host->cmd) 
			{
				...
				{
#ifdef MMC_DEBUG
					printk(KERN_ERR "MMCSD: Command timeout, CMD%d and status is %x\r\n",host->cmd->opcode,status);
#endif				
				}
				host->cmd->error |= MMC_ERR_TIMEOUT;
				end_command = 1;
		...
		if(host->data ==NULL)
		{
			...
			{
#ifdef MMC_DEBUG			
				printk("Status is %x at end of ISR when host->data is NULL",status);
#endif			
				status = 0;
...
static int davinci_mmcsd_probe(struct device *dev)
{
	...
	mmcsdregs = (mmcsd_regs *)DAVINCI_PERI_ADDR(MMCSD_REGS_BASE_ADDR);
	initMmcsdHost();
	...
	host->sd_support =1;
	ret = request_irq(MMCINT_INTERRUPT, mmc_davinci_irq, 0, DRIVER_NAME, host);
...
static struct resource mmc_resources[] = {
	{
		.start		= DAVINCI_PERI_ADDR(MMCSD_REGS_BASE_ADDR),
		.end		= DAVINCI_PERI_ADDR((MMCSD_REGS_BASE_ADDR) + 0x74),

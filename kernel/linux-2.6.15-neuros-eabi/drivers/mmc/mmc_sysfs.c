...
#define add_env(fmt,val)						\
	...
	if(card->host->mode == MMC_MODE_MMC)
	  {
	      printk("It is MMC card\n");
	      add_env("CARD_MODE=%s", "mmc");
	  }
	else if(card->host->mode == MMC_MODE_SD)
	  {  
	      printk("It is SD card\n");
	      add_env("CARD_MODE=%s", "sd");
	  }
...
EXPORT_SYMBOL(mmc_register_card);
...
EXPORT_SYMBOL(mmc_alloc_host_sysfs);
...
EXPORT_SYMBOL(mmc_add_host_sysfs);
...
EXPORT_SYMBOL(mmc_remove_host_sysfs);
...
EXPORT_SYMBOL(mmc_free_host_sysfs);

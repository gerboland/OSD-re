...
#include <asm/arch/itmmc.h>
#include <asm/semaphore.h>

#include "itomap243x_mmc.h"
...
#define MMC_CARDDETECT_GPIO_IRQ OMAP24XX_MMC_CD_IRQ
...
void mmcpower_slot_enable(unsigned char reg, unsigned char val)
{
    ...
    omap_set_gpio_dataout(OMAP24XX_MMC_EN_PWR, 1);
#else
...
mmc_omap_start_command(struct mmc_omap_host *host, struct mmc_command *cmd)
{
    ...
    DBG("%s: CMD%d, argument 0x%08x", DRIVER_NAME, cmd->opcode,
        cmd->arg);
...
static irqreturn_t mmc_omap_irq_cd(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    card_status = omap_get_gpio_datain(MMC_CARDDETECT_GPIO_IRQ);
                       ...
                       struct mmc_request *req)
{
    ...
    if (!(req->data->flags & MMC_DATA_WRITE)) {
        sync_dev = OMAP24XX_DMA_MMC_RX;
        dev_name = "MMC1 read";
    } else {
        sync_dev = OMAP24XX_DMA_MMC_TX;
        dev_name = "MMC1 write";
    }
    ...
    if (!(req->data->flags & MMC_DATA_WRITE)) {
        ...
        omap_set_dma_src_params(dma_ch, 0x00, MMC_SRC, 0, 0, 0);
        ...
        omap_set_dma_dest_params(dma_ch, 0x00, MMC_SRC, 0, 0, 0);
...
static void omap24xx_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    DBG("%s: set_ios: clock %dHz busmode %d powermode %d Vdd %x\n",
        DRIVER_NAME, ios->clock, ios->bus_mode, ios->power_mode,
        ios->vdd);
...
static int omap24xx_mmc_probe(struct device *dev)
{
    struct mmc_host *mmc;
    ...
    if (ret) {
        printk("Unable to grab HSMMC IRQ\n");
        return ret;
    }

    set_irq_type(MMC_CARDDETECT_GPIO_IRQ, IRQT_BOTHEDGE);
    ret = request_irq(MMC_CARDDETECT_GPIO_IRQ, mmc_omap_irq_cd,
        SA_INTERRUPT, DRIVER_NAME, host);
    if (ret) {
        printk("Unable to grab mmc card detect interrupt\n");
...
static int omap24xx_mmc_remove(struct device *dev)
{
...
#ifdef CONFIG_PM
static int omap24xx_mmc_suspend(struct device *dev, u32 state, u32 level)
{
...
static int omap24xx_mmc_resume(struct device *dev, u32 level)
{
    struct mmc_omap_host *host = omap_get_drvdata(dev);
    struct clk *hsmmci, *hsmmcf;
    int ret = 0;
...
static struct device_driver itomap2430_mmc_driver = {
    .name       = DRIVER_NAME,
...
//  struct clk *hsmmci, *hsmmcf;
    ...
    printk("mmc omap243x driver module 1\n");
#elif defined(CONFIG_OMAP2430_MMC2)
    printk("mmc omap243x driver module 2\n");
#endif
...
//  clk_safe(hsmmcf);
    ...
    if (driver_register(&itomap2430_mmc_driver)) {
...
//  clk_unuse(hsmmcf);
#endif
    driver_unregister(&itomap2430_mmc_driver);
}

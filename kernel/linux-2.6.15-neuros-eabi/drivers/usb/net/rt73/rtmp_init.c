	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (Status != NDIS_STATUS_SUCCESS)
	{
		...
		memcpy(pFirmwareImage, &FirmwareImage[0], FileLength);
			...
			if(RTMPGetKeyParameter(tok_str, tmpbuf, 128, buffer))
			{
                ...
                {//Ascii
                    if( (KeyLen == 5) || (KeyLen == 13))
                    {
                        ...
                        memcpy(pAd->SharedKey[idx].Key, tmpbuf, KeyLen);
                        if (KeyLen == 5)
                            CipherAlg = CIPHER_WEP64;
                        else
                            CipherAlg = CIPHER_WEP128;
			...
			if (srcf->f_op && srcf->f_op->read)
			{
				...
				else
				{
					     ...
					     (pAd->PortCfg.AuthMode == Ndis802_11AuthModeAutoSwitch)))
					{
					    ...
					    memcpy(pAd->PortCfg.DesireSharedKey[KeyIdx].Key, pAd->SharedKey[KeyIdx].Key, pAd->SharedKey[KeyIdx].KeyLen);

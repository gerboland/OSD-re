...
MODULE_PARM_DESC(debug, "Enable debug, accepted values: 0 (no debug, default), 1 (Trace), 2 (Info).");
...
INT __init usb_rtusb_init(void)
{
    printk("rtusb init ====>\n");
    	...
    	switch (debug)
	{
		case 0:			//No need to set
			break;
		case 1:
			RTDebugLevel = RT_DEBUG_TRACE;
			break;
		case 2:
			RTDebugLevel = RT_DEBUG_INFO;
			break;
		default:		//Default: no debug
			break;
	}
	#endif

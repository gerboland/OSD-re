...
#ifdef  CONFIG_USB_GADGET_DM320 
#define CHIP			"dm320"
#endif
 
#ifdef  CONFIG_USB_GADGET_NET2272_PLATFORM  
#define CHIP			"net2272"
#endif
...
#define DBG(dev,fmt,args...) \
	xprintk(dev , KERN_EMERG , fmt , ## args)
#else
#define DBG(dev,fmt,args...) \
	do { } while (0)
#endif /* DEBUG */
...
static void dump_msg(char *buf, unsigned int length)
{
...
ep_read (struct file *fd, char __user *buf, size_t len, loff_t *ptr)
{
	...
	if (data->desc.bEndpointAddress & USB_DIR_IN) {
		...
		{
			printk("Wrong direction of Data transfer\n");
			return -EINVAL;
		}
		DBG (data->dev, "%s halt\n", data->name);
	...
	//printk("%s: %s read %d OUT, status %d\n",__FILE__,data->name, len, value);
	//dump_msg(kbuf,len);
...
ep_write (struct file *fd, const char __user *buf, size_t len, loff_t *ptr)
{
	...
	if ((value = get_ready_ep (fd->f_flags, data)) < 0)
		return value;
	printk("%s: Write to endpoint IN\n", __FUNCTION__);
	/* halt any endpoint by doing a "wrong direction" i/o call */
	if (!(data->desc.bEndpointAddress & USB_DIR_IN)) {
		if ((data->desc.bmAttributes & USB_ENDPOINT_XFERTYPE_MASK)
				== USB_ENDPOINT_XFER_ISOC)
		{
			printk(KERN_EMERG "Wrong direction of Data transfer\n");
			return -EINVAL;
		}
		DBG (data->dev, "%s halt\n", data->name);
		spin_lock_irq (&data->dev->lock);
		if (likely (data->ep != NULL))
		{
			printk(KERN_EMERG "%s: Halting endpoint!!\n", __FUNCTION__);
			usb_ep_set_halt (data->ep);
		}
		spin_unlock_irq (&data->dev->lock);
		up (&data->lock);
	...
	if (!kbuf)
	{
		printk("%s: Could not allocate buffer of size %d\n",__FUNCTION__,len);
		goto free1;
	}
	if (copy_from_user (kbuf, buf, len)) {
		printk("%s: Could not copy buffer of size %d from user space\n",__FUNCTION__,len);
		value = -EFAULT;
		goto free1;
	}
	value = ep_io (data, kbuf, len);
	printk("%s: %s write %d IN, status %d\n",__FUNCTION__,data->name, len, value);
	//dump_msg(kbuf,len);

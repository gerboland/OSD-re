...
static u8 MGC_HsDmaProgramChannel(MGC_DmaChannel * pChannel,
...
static MGC_DmaController *MGC_HsNewDmaController(MGC_pfDmaChannelStatusChanged
...
static u8 MGC_HsDmaControllerIsr(void *pPrivateData)
{
	...
	MGC_HsDmaController *pController = (MGC_HsDmaController *) pPrivateData;
	u8 *pBase = pController->pCoreBase;
	u8 bIntr = MGC_Read8(pBase, MGC_O_HSDMA_INTR);
						 ...
						 u8 * pCoreBase)
{
	...
	if (pController) {
		...
		pController->Controller.pfDmaGetChannelStatus =

 ...
 *                                  regions.
 *   01/14/05  Jay Williams       - Optimized driver for IT-DM320 
 */
...
#if (defined(CONFIG_NTDEV_DM320) || defined(CONFIG_ARCH_NTR3_DM320) || defined(CONFIG_ARCH_ITDM320_20)) &&\
    defined(CONFIG_DMA_NET)
static struct sk_buff *next_skb = NULL, *prev_skb = NULL;
#endif
...
#define PRINTK(args...)   printk(KERN_INFO args)
#endif
...
#if (defined(CONFIG_ARCH_NTDEV_DM320) || defined(CONFIG_ARCH_NTR3_DM320) || defined(CONFIG_ARCH_ITDM320_20)) &&\
    defined(CONFIG_DMA_NET)
    // Used to improve rx performance (JJW)
    next_skb = dev_alloc_skb(1500);
    arm926_dma_flush_range((int) next_skb->tail, (int) (next_skb->tail + 1500));
#endif
}
...
#if (defined(CONFIG_ARCH_NTDEV_DM320) || defined(CONFIG_ARCH_NTR3_DM320) || defined(CONFIG_ARCH_ITDM320_20)) &&\
    defined(CONFIG_DMA_NET)
		//arm926_dma_flush_range((int) data, (int) (data + data_len));
		SMC_DM320_DMA_insw(ioaddr,
				   DATA_REG,
				   data,
				   (packet_len - 2) >> 1);

		if (prev_skb) {
			prev_skb->protocol = eth_type_trans(prev_skb, dev);
			netif_rx(prev_skb);
		}
		next_skb = dev_alloc_skb(1500);

		arm926_dma_flush_range((int) next_skb->tail,
				       (int) (next_skb->tail + 1500));
...
#if (defined(CONFIG_ARCH_NTDEV_DM320) || defined(CONFIG_ARCH_NTR3_DM320) || defined(CONFIG_ARCH_ITDM320_20)) &&\
    defined(CONFIG_DMA_NET)
    // Check to see if we need to commit RX buffer
    if (prev_skb) {
       prev_skb->protocol = eth_type_trans(prev_skb, dev);
       netif_rx(prev_skb);
    }
...
static int __init smc_probe(struct net_device *dev, unsigned long ioaddr)
{
	...
	/* Grab the IRQ */
    retval = request_irq(dev->irq, &smc_interrupt, SA_INTERRUPT, dev->name, dev);
    if (retval)
       goto err_out;
...
#if defined(CONFIG_ARCH_NTDEV_DM320) || defined(CONFIG_ARCH_NTR3_DM320) || defined(CONFIG_ARCH_ITDM320_20) || defined(CONFIG_ARCH_DCIII)
	gio_disable_irq(GIO_ETHER);
#endif

...
#define SDDMASEL_VAL  0x0830 /* DMASEL3=MS, DMASEL2=SIF, DMASEL1=MTC 8 burst */ 
#define MTCADDRH_MASK 

#ifdef CONFIG_DMA_NET
#define TIMING_CALCULATION 1
#endif
#define PLATFORM_CACHE_LINE_BYTES 32
#define PLATFORM_FIFO_SEL_ADDR 0x10000 /* A15 line of word address  */
#define NET_DMA_INIT()
...
#include <asm/arch/gio.h>
#include <asm/arch/irqs.h>
#define SDDMASEL_VAL 0x0210 /* DMASEL3=MS, DMASEL2=SIF, DMASEL1=MTC */ 
#define PLATFORM_CACHE_LINE_BYTES 32
#define PLATFORM_FIFO_SEL_ADDR 0x10000 /* A15 line of word address  */
#define NET_DMA_INIT() \
                outw(0x4, IO_EMIF_DMAXSIZE ); \
                outw(0x4, IO_EMIF_ARMOFFSET ); \
                outw(0x4, IO_EMIF_MTCOFFSET ); \
...
#define PLATFORM_CACHE_LINE_BYTES 32
#define PLATFORM_FIFO_SEL_ADDR 0x100 /* A6 line of longword address bus */

#define PINMUX_DECLARE( x )    spinlock_t x ## _spinlock = SPIN_LOCK_UNLOCKED; unsigned long x
#define PINMUX_SAVE( x )       do { spin_lock( &x ## _spinlock ); (x) = pinmux_emif_save(); } while (0)
#define PINMUX_RESTORE( x )    do { pinmux_restore(x); spin_unlock( &x ## _spinlock ); } while (0)

#endif

#ifndef PINMUX_DECLARE  /* If platform has no pin multiplexing, do nothing */
#define PINMUX_DECLARE( x )
#define PINMUX_SAVE( x )
#define PINMUX_RESTORE( x )
#endif
...
#define Lan_SetRegDW(dwOffset,dwVal) \
	do { ((*(volatile DWORD *)(privateData->dwLanBase+dwOffset))=(dwVal)); } while (0)
#define Lan_ClrBitsDW(dwOffset,dwBits) \
	do { ((*(volatile DWORD *)(privateData->dwLanBase+dwOffset))&=(~dwBits)); } while (0)
...
#define CONFIG_SMC9118_BASE 0x04000000 /* CS3 */
#endif
#endif

#if defined(CONFIG_ARCH_ITDM3XX) || defined (CONFIG_ARCH_DAVINCI)
DWORD lan_base[MAX_INSTANCES]=
		{CONFIG_SMC9118_BASE ,0x00UL,0x00UL,0x00UL,
...
#if defined(CONFIG_ARCH_ITDM3XX)
DWORD irq[MAX_INSTANCES]=
		{IRQ_GIO10,0x00UL,0x00UL,0x00UL,
		 0x00UL,0x00UL,0x00UL,0x00UL};
const char* irq_names[MAX_INSTANCES]={"SMSC_911x_0", "SMSC_911x_1" ,"SMSC_911x_2", "SMSC_911x_3", 
                          "SMSC_911x_4", "SMSC_911x_5" ,"SMSC_911x_6", "SMSC_911x_7"};
#elif defined(CONFIG_ARCH_DAVINCI)
DWORD irq[MAX_INSTANCES]=
                 {IRQ_GPIO3/*DAVINCI_GPIO3_INTERRUPT*/,0x00UL,0x00UL,0x00UL,
		 0x00UL,0x00UL,0x00UL,0x00UL};
const char* irq_names[MAX_INSTANCES]={"SMSC_911x_0", "SMSC_911x_1" ,"SMSC_911x_2", "SMSC_911x_3", 
                          "SMSC_911x_4", "SMSC_911x_5" ,"SMSC_911x_6", "SMSC_911x_7"};
#else
...
int Simp911x_init_module(void)
{
	...
	if (request_gio(GIO_ETHER))
		return -ENODEV;
   	SMSC_TRACE("request_gio success\n"); 
#ifdef TIMING_CALCULATION
	if (request_gio(GIO_USB_ENABLE))
		return -ENODEV;
	gio_set_dir(GIO_USB_ENABLE, bit_low);

	if (request_gio(GIO_HDD_HOTPLUG))
		return -ENODEV;
	gio_set_dir(GIO_HDD_HOTPLUG, bit_low);
...
#if defined(CONFIG_DMA_NET)
	/* platform-specific DMA initialization */ 
	NET_DMA_INIT();
#endif
...
#if defined(CONFIG_ARCH_ITDM3XX) || defined( CONFIG_ARCH_DAVINCI )
	unrequest_gio(GIO_ETHER);
#ifdef TIMING_CALCULATION
	unrequest_gio(GIO_USB_ENABLE);
	unrequest_gio(GIO_HDD_HOTPLUG);
#endif
#endif
...
#if defined(CONFIG_ARCH_ITDM3XX) || defined( CONFIG_ARCH_DAVINCI )
	privateData->dwLanBase=(DWORD )ioremap(lan_base[gRegistrationIndex],0xFC);
#else
...
int Simp911x_open(struct net_device *dev)
{
	...
	// Get MAC address from part before we reset the part in Lan_Initialize().
	// This is helpful in the case where no EEPROM is fitted, and the 
	// boot code initialized the MAC - John Whittington
	...
	//initialize the LAN911x
	{
		...
		const DWORD dwIntCfg=0;//set interrupt deassertion to 220uS
	...
	if(request_irq(
		...
		Simp911x_ISR,
		SA_INTERRUPT,
		irq_names[privateData->dwInstanceIndex],
		privateData)!=0)
	{
		result=-ENODEV;
		goto DONE;
...
SMSC_TRACE("Changed to falling edge");
	gio_enable_irq(GIO_ETHER, GIO_FALLING_EDGE);
	...
	privateData->NextSkb = dev_alloc_skb(1500+2*PLATFORM_CACHE_LINE_BYTES); 
	arm926_dma_flush_range((DWORD) privateData->NextSkb->tail,
                                       (DWORD) (privateData->NextSkb->tail+1500));
...
#ifdef TIMING_CALCULATION
	gio_set_bitset(GIO_HDD_HOTPLUG);
#endif
        PINMUX_SAVE( pinmux );
...
#ifdef TIMING_CALCULATION
	gio_set_bitclr(GIO_HDD_HOTPLUG);
#endif
...
void Phy_CheckLink(unsigned long ptr)
{
	PPRIVATE_DATA privateData=(void *)ptr;
    PINMUX_DECLARE( pinmux );
	if(privateData==NULL) {
...
#if defined(CONFIG_DMA_NET) && defined(CONFIG_DMA_NET_TX)
#define SMC_DM320_DMA_outsw(ioaddr, reg, data, word) \
        ({\
                volatile u16 *d_ptr;\
                u_long dest_addr, net_addr;\
                unsigned int wordlen = word;\
\
                d_ptr = (volatile u16*) data;\
                net_addr = virt_to_phys((void*) d_ptr);\
                if (((u32) net_addr) & 0x2) {\
			printk(" not 4 byte aligned \n");\
                        writesw(ioaddr + reg, data, 1);\
                        dest_addr += 2;\
                        wordlen -= 2;\
                }\
                if (!wordlen) return;\
		dest_addr =  reg;  \
                outw(SDDMASEL_VAL, IO_SDRAM_SDDMASEL);/* MTC 1 burst DMA */\
                outw(0x0004, IO_EMIF_DMAMTCSEL);/* DMA MTC Select: CS4*/\
                outw(((dest_addr & 0x0FFF0000) >> 16) , IO_EMIF_MTCADDH);\
...
#ifdef CONFIG_DMA_NET
#define SMC_DM320_DMA_WAIT() \
        do {\
                volatile u16 val;\
\
                val = inw(IO_EMIF_DMACTL);\
\
                if ((val & 0x01) == 0) break;\
        } while(1)
#endif
	...
	struct sk_buff *skb)
{
 ...
 //prepare for 16 byte alignment
	...
	dwWordCount = ((((DWORD)(skb->len))+(PLATFORM_CACHE_LINE_BYTES-1)+
                      (((DWORD)(skb->data))&(PLATFORM_CACHE_LINE_BYTES-1)))&
                      (~(PLATFORM_CACHE_LINE_BYTES-1)))>>2;

	arm926_dma_flush_range((DWORD)skb->data,(DWORD)(skb->data + (dwWordCount<<2)));

#if defined(CONFIG_ARCH_ITDM320_20)
	SMC_DM320_DMA_outsw(privateData->dwLanBase,(CONFIG_SMC9118_BASE | 0x10000),
                   (((DWORD)(skb->data))&(~(PLATFORM_CACHE_LINE_BYTES-1))),
                   dwWordCount);
#else
	SMC_DM320_DMA_outsw(privateData->dwLanBase,
			    PLATFORM_FIFO_SEL_ADDR,
			    (((DWORD)(skb->data))&(~(PLATFORM_CACHE_LINE_BYTES-1))),
		  dwWordCount);
#endif
...
#if defined(CONFIG_DMA_NET) && defined(CONFIG_DMA_NET_RX)
	SMC_DM320_DMA_WAIT();
#endif
	result=netif_rx(skb);
...
void Rx_CountErrors(PPRIVATE_DATA privateData,DWORD dwRxStatus) 
{
	...
	if(!crcError) {
		if( (dwRxStatus&0x00001020UL) == 0x00001000UL ) {
...
#if defined(CONFIG_DMA_NET) && defined(CONFIG_DMA_NET_RX)
#define SMC_DM320_DMA_insw(ioaddr, reg, data, word) \
        ({\
                volatile u16 *d_ptr;\
                u_long dest_addr, net_addr;\
                unsigned int wordlen = word;\
\
			...
			printk(" not 4 byte aligned \n"); \
                ...
                outw(SDDMASEL_VAL, IO_SDRAM_SDDMASEL);/* MTC 1 burst DMA */\
                outw(0x0004, IO_EMIF_DMAMTCSEL);/* DMA MTC Select: CS4*/\
                outw(((net_addr & 0x0FFF0000) >> 16)  , IO_EMIF_MTCADDH);\
                ...
                outw((wordlen + 1) >> 1, IO_EMIF_DMASIZE);\
                /* Start the DMA transfer */\
...
void Rx_ProcessPackets(PPRIVATE_DATA privateData)
{
...
#if defined(CONFIG_ARCH_ITDM3XX) &&  defined(CONFIG_DMA_NET) && defined(CONFIG_DMA_NET_RX)
#ifdef PROCESS_ALL_PACKETS
	...
	gio_set_bitset(GIO_USB_ENABLE);
		...
		if((dwRxStatus&RX_STS_ES_)==0) {
			...
			skb=privateData->NextSkb;
			if(skb!=NULL) {
				//use DMA
                                DWORD dwDwordCount;
				skb->data=skb->head;
				skb->tail=skb->head;
#ifdef FOUR_BYTES_ALIGNMENT
				...
				dwDwordCount=((dwPacketLength+
                                                        ...
                                                        (~(PLATFORM_CACHE_LINE_BYTES-1)))>>2;
#ifdef VERBOSE_DEBUG
				printk("\n Before alignment %x %x len %d dwDwordCount %d ",
				skb->head,skb,dwPacketLength,dwDwordCount);
#endif
				//update counters
 ...
 //set end alignment and offset
                                                ...
                                                {
                                                case 4: Lan_SetRegDW(RX_CFG,0x00000200UL);
                                                //case 4: Lan_SetRegDW(RX_CFG,0x00000200UL |  (dwPacketLength >> 16));
                                                case 16:Lan_SetRegDW(RX_CFG,0x40000200UL);break;
                                                case 32:Lan_SetRegDW(RX_CFG,0x80001200UL);break;
                                                default:SMSC_ASSERT(FALSE);
                                                }

#if defined(CONFIG_ARCH_ITDM320_20) || defined (CONFIG_ARCH_NTDEV_DM320) || defined (CONFIG_ARCH_NTR3_DM320)
 SMC_DM320_DMA_insw(privateData->dwLanBase,
                                                 (CONFIG_SMC9118_BASE | 0x10000),
                                                 (skb->head),
                                                 dwDwordCount << 1);
#else
 						 SMC_DM320_DMA_insw(privateData->dwLanBase,
...
//Allocate next skb,
						...
						/* JTW - only invalidation is necessary for input packets */
						//arm926_dma_flush_range((DWORD) privateData->NextSkb->tail,
						arm926_dma_inv_range((DWORD) privateData->NextSkb->tail,

                                       		(DWORD) (privateData->NextSkb->tail+1500));
#ifdef VERBOSE_DEBUG
						PRINT_PKT(skb->data,dwPacketLength);
						printk("\n size  %d skb->len %d ",dwPacketLength,skb->len);
#endif
...
#define REV_A_WORK_AROUND 1
#ifdef REV_A_WORK_AROUND
//Reset workaround
	SMSC_TRACE(" REV_A Reset work around - Reset 2");
	udelay(10);
	//Reset the LAN911x
	Lan_SetRegDW(HW_CFG,HW_CFG_SRST_);
	dwTimeOut=10;
	...
	SMSC_TRACE(" REV_A Reset work around - Reset 3");
	udelay(10);
	//Reset the LAN911x
	Lan_SetRegDW(HW_CFG,HW_CFG_SRST_);
	dwTimeOut=10;
	...
	//Lan_SetRegDW(HW_CFG,0x00030000UL);

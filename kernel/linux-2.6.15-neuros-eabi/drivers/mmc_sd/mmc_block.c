...
//          mmc_blk_readonly(md->queue.card))
        ...
        if (mmc_card_dead(md->queue.card) && 
            mmc_card_dead(md->queue.card))
        {
            printk ("card dead\n");    
            ret = -ENXIO;
        }
...
#define CARD_PRESENT    0x55
#define CARD_DEAD       0x44
static int
...
static int mmc_blk_prep_rq(struct mmc_queue *mq, struct request *req)
{
    ...
    {
        if (!md || !mq->card || mmc_card_dead(mq->card)) 
        {
            //printk(KERN_ERR "%s: killing request - no device/host\n",req->rq_disk->disk_name);
            stat = BLKPREP_KILL;
        }

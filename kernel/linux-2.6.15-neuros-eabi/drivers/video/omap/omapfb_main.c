...
extern struct lcd_panel itomap2430_sharp01_panel;
extern struct lcd_panel itomap2430_casio01_panel;
extern struct lcd_panel itomap2430_ntsc_m_panel;
extern struct lcd_panel itomap2430_ntsc_j_panel;
extern struct lcd_panel itomap2430_ntsc_443_panel;
extern struct lcd_panel itomap2430_pal_bdghi_panel;
extern struct lcd_panel itomap2430_pal_nc_panel;
extern struct lcd_panel itomap2430_pal_n_panel;
extern struct lcd_panel itomap2430_pal_m_panel;
extern struct lcd_panel itomap2430_pal_60_panel;
...
#ifdef CONFIG_MACH_ITOMAP2430
# if defined(CONFIG_FB_OMAP_LCDC_INTERNAL)
#  if defined(CONFIG_FB_OMAP_LCDC_SHARP01)
    &itomap2430_sharp01_panel,
#  elif defined(CONFIG_FB_OMAP_LCDC_CASIO01)
    &itomap2430_casio01_panel,
#  endif
# endif
# if defined(CONFIG_FB_OMAP_TVC_INTERNAL)
#  if defined(CONFIG_FB_OMAP_TVC_NTSC_M_PANEL)
    &itomap2430_ntsc_m_panel,
#  elif defined(CONFIG_FB_OMAP_TVC_NTSC_J_PANEL)
    &itomap2430_ntsc_j_panel,
#  elif defined(CONFIG_FB_OMAP_TVC_NTSC_443_PANEL)
    &itomap2430_ntsc_443_panel,
#  elif defined(CONFIG_FB_OMAP_TVC_PAL_BDGHI_PANEL)
    &itomap2430_pal_bdghi_panel,
#  elif defined(CONFIG_FB_OMAP_TVC_PAL_NC_PANEL)
    &itomap2430_pal_nc_panel,
#  elif defined(CONFIG_FB_OMAP_TVC_PAL_N_PANEL)
    &itomap2430_pal_n_panel,
#  elif defined(CONFIG_FB_OMAP_TVC_PAL_M_PANEL)
    &itomap2430_pal_m_panel,
#  elif defined(CONFIG_FB_OMAP_TVC_PAL_60_PANEL)
    &itomap2430_pal_60_panel,
#  endif
# endif
#endif
};
...
#if defined(CONFIG_FB_OMAP_LCDC_INTERNAL) || \
    defined(CONFIG_FB_OMAP_TVC_INTERNAL)
...
static int ctrl_change_mode(struct omapfb_device *fbdev)
{
    ...
    {
        r = fbdev->ctrl->setup_plane(OMAPFB_PLANE_GFX, OMAPFB_CHANNEL_OUT_DIGIT,
            offset, var->xres_virtual, 0, 0, var->xres,
            var->yres, fbdev->color_mode);
    }
    else
...
static int omapfb_find_panel(struct omapfb_device *fbdev)
{
    ...
    strncpy(display, conf->sel_display, sizeof(display) - 1);
    display[sizeof(display) - 1] = 0;
    if (strcmp(display, "lcd") == 0)
    {
        omapfb_disp_type = OMAP_LCDC_TYPE;
    }
    else if (strcmp(display, "tv") == 0)
    {
        omapfb_disp_type = OMAP_TVC_TYPE;
    }
    else
    {
        return -1;
    ...
    for (i = 0; i < ARRAY_SIZE(panels); i++) {
        if ((strcmp(panels[i]->name, name) == 0) && 
            (omapfb_disp_type == panels[i]->display_type)) {

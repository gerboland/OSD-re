/*
 * File: drivers/video/omap/lcd-itomap2430-ntsc_j.c
 *
...
static int itomap2430_ntsc_j_panel_init(struct omapfb_device *fbdev)
{
    int status;
    DBGENTER(1);
    status = write_venc(&ntsc_j_cfg);
    DBGLEAVE(1);
    return(status);
}

static void itomap2430_ntsc_j_panel_cleanup(void)
{
    DBGENTER(1);
    DBGLEAVE(1);
}

static int itomap2430_ntsc_j_panel_enable(void)
{
...
static void itomap2430_ntsc_j_panel_disable(void)
{
    DBGENTER(1);
    DBGLEAVE(1);
}

static unsigned long itomap2430_ntsc_j_panel_get_caps(void)
{
    return 0;
}

struct lcd_panel itomap2430_ntsc_j_panel = {
    .name           = "itomap2430",
    ...
    .init           = itomap2430_ntsc_j_panel_init,
    .cleanup        = itomap2430_ntsc_j_panel_cleanup,
    .enable         = itomap2430_ntsc_j_panel_enable,
    .disable        = itomap2430_ntsc_j_panel_disable,
    .get_caps       = itomap2430_ntsc_j_panel_get_caps,
};
